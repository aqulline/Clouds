...
for program, modules in data.items():
    ...
    html_table += '<thead><tr><th>Module</th><th>Venue</th><th>Teacher</th><th>Day</th><th>Time</th><th>Week</th><th>Students Size</th><th>Venue Size</th></tr></thead>'
    for module, details in modules.items():
        html_table += '<tbody>'
        html_table += '<tr>'
        html_table += f'<td>{module}</td>'
        html_table += f'<td>{details["venue"]}</td>'
        html_table += f'<td>{details["teacher"]}</td>'
